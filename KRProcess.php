...
function SizesValadation($NumExpexted, $input){
    ...
    if ($lenInputArr !== $NumExpexted - 1 ){
        echo $lenInputArr . "<br>" . $NumExpexted . "<br>";
        $msg = "Either Too Few Or Too Many Sizes Given For Selected Item!";
        $_SESSION['msg'] = $msg;
        ...
        while ($loop < $lenInputArr){
            ...
            try{
                $int = (int)$inputArr[$loop];
                
            }catch(Exception $e){
                $msg = "Sizes Must Only Contain Intigers Seperated By A /";
                $_SESSION['msg'] = $msg;
                header("location: kitRequest.php");
            }
            // checks if the sizes is too long
            if (strlen($inputArr[$loop])>3){
                $msg = "Too Large Of A Size Inputed To Be Accepted";
                $_SESSION['msg'] = $msg;
                header("location: kitRequest.php");
            }else{
...
if ($UniformType == "" or $Size == "" or $purpose == "" or $NumRequested == "" ){
    ...
    SizesValadation($NumSizesExpected, $Size);
